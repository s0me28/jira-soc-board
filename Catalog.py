...
def add_grade():
    ...
    action = ('add', grade, datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
...
def edit_grade():
    ...
    if student_id in grades and subject in grades[student_id]:
        ...
        action = ('edit', old_grade, new_grade, datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        if student_id not in history:
            history[student_id] = {}
        if subject not in history[student_id]:
            history[student_id][subject] = []
...
def delete_grade():
    ...
    if student_id in grades and subject in grades[student_id]:
        ...
        action = ('delete', grade_to_delete, datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        if student_id not in history:
            history[student_id] = {}
        if subject not in history[student_id]:
            history[student_id][subject] = []
...
def student_dashboard(student_id):
    ...
    for subject, subject_grades in student_grades.items():
        if subject_grades:
            ...
            student_averages[subject] = average
        else:
            student_averages[subject] = None

    student_history = history.get(student_id, {})
